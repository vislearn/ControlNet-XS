...
from data.datam import DataModuleFromConfig as DataModuleFromConfig
...
def get_parser(**parser_kwargs):
    ...
    # 'gradient_clip_algorithm': None, 'num_nodes': 1, 
    # 'num_processes': None, 'devices': None, 'gpus': None, 
    # 'auto_select_gpus': None, 'tpu_cores': None, 'ipus': None, 
    # 'enable_progress_bar': True, 'overfit_batches': 0.0, 'track_grad_norm': -1, 
    # 'check_val_every_n_epoch': 1, 'fast_dev_run': False, 
    # 'accumulate_grad_batches': None, 'max_epochs': None, 
    # 'min_epochs': None, 'max_steps': -1, 'min_steps': None, 
    # 'max_time': None, 'limit_train_batches': None, 
    # 'limit_val_batches': None, 'limit_test_batches': None, 
    # 'limit_predict_batches': None, 'val_check_interval': None, 
    # 'log_every_n_steps': 50, 'accelerator': None, 'strategy': None, 
    # 'sync_batchnorm': False, 'precision': 32, 'enable_model_summary': True, 
    ...
    def on_fit_start(self, trainer, pl_module):
        if trainer.global_rank == 0:
            ...
            if MULTINODE_HACKS:
                ...
                time.sleep(5)
            print(f">>-self.cfgdir {self.cfgdir}")
            OmegaConf.save(
                self.config,
...
if __name__ == "__main__":
    ...
    # 根据logdir创建了两个目录：ckptdir用于保存模型检查点，cfgdir用于保存配置文件。
    ...
    try:
        ...
        print(f"OmegaConf.create(), {OmegaConf.create()}")
        # default to gpu
        ...
        # 包括合并多个配置文件、处理命令行参数、提取训练器配置等。

        # 然后，根据配置文件中的model配置实例化模型。 ------
        print(f">> config :{config}")
        # model
        model = instantiate_from_config(config.model)
        print(f">> model type is inhrint from pllight_module?")

        # trainer and callbacks
        trainer_kwargs = dict()

        # default logger configs
        # 接下来是关于训练器（trainer）和回调函数（callbacks）的设置。代码中给出了两个默认的日志记录器（logger）配置，分别是wandb和csv。这些配置定义了日志记录器的类型以及其参数
        default_logger_cfgs = {
            "wandb": {
        ...
        # 包括目录路径（dirpath）、文件名模板（filename）、是否显示详细信息（verbose）和是否保存最后一个检查点（save_last）等参数
        ...
        # 接下来，检查模型是否具有monitor属性（用于确定最佳模型的指标），如果有，则将其作为检查点指标（monitor）以及保存的最佳模型数（save_top_k）添加到模型检查点配置中。
        if hasattr(model, "monitor"):
            print(f"Monitoring {model.monitor} as checkpoint metric.")
            default_modelckpt_cfg["params"]["monitor"] = model.monitor
            default_modelckpt_cfg["params"]["save_top_k"] = 3
        
        # 然后，判断配置文件中是否包含modelcheckpoint字段，如果有，则将其作为模型检查点的配置（modelckpt_cfg）；否则，创建一个空的配置。然后，将默认的模型检查点配置和配置文件中的模型检查点配置合并。
        ...
        # default to ddp if not further specified ----定义了一个默认的训练策略配置default_strategy_config，
        ...
        # 根据合并后的训练策略配置实例化一个训练策略对象，并将其添加到训练器参数（trainer_kwargs）中。
        ...
        ds = load_from_disk("/home/dell/workspace/dataset/test_lambdalabs___pokemon-blip-captions/")
        # config["test"] = ds
        # data.test_dataloader = partial(data._test_dataloader, shuffle=shuffle_test_loader)
        sample_dataloader = torch.utils.data.DataLoader(ds, batch_size=2)
        print(">> all test data ", len(sample_dataloader))
        print("####-----> Data #####")
        try:
        ...
        # trainer.test(model, sample_dataloader)
        if opt.train:
            try:
